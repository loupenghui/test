...
public class VerfiyIDCard {
	...
	public static void main(String[] args) {
		String cardId = "341621199107232720";
		if (cardId.length() == 15 || cardId.length() == 18) {
			if (!cardCodeVerifySimple(cardId)) {
				System.err.println("15位或18位身份证号码不正确");
			} else if (cardId.length() == 18 && !cardCodeVerify(cardId)) {
				System.err.println( "18位身份证号码不符合国家规范");
			} else {
				System.out.println("正确");
			}
		} else {
			System.err.println("身份证号码长度必须等于15或18位");
		}
	}
}
